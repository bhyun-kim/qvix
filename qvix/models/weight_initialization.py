...
def initialize_layer(model: eqx.Module,
    ...
    get_weights = lambda m: [x.weight
                            for x in jax.tree_util.tree_leaves(m, is_leaf=is_target)
                            if is_target(x)]
    weights = get_weights(model)

    initializer_name = init_cfg['initializer']
    if hasattr(jax.nn.initializers, initializer_name):
        initializer = getattr(jax.nn.initializers, initializer_name)(**init_cfg['kwargs'])
    else:
...
def initialize_model(model: eqx.Module, 
                     key: jax.random.PRNGKey,
